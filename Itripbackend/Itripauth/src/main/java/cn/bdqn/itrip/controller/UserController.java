...
public class UserController {
    ...
    public Dto login(String name, String password, String token, HttpServletRequest request){
        ItripUser user= null;
        String newtoken="";
        try {
            ...
            //key为token=md5(用户名ID+用户Code+过期时间)
            //value用户的对象转成字符串 fastJson
            newtoken=Token(user, request.getHeader("User-Agent"));
            //往redis数据库中添加一条数据，key值为newtoken
            redisHelp.setRedis(newtoken,60*60*2 ,JSONArray.toJSONString(user));
        } catch (Exception e) {
            e.printStackTrace();
        }
        ItripTokenVO tokenVO=new ItripTokenVO(token,Calendar.getInstance().getTimeInMillis()+60*60*2,Calendar.getInstance().getTimeInMillis());
    ...
    public @ResponseBody Dto register(@RequestBody ItripUserVO userVO){
        ...
        try{
            ...
            //第一步插入到数据库中，未激活
            int i= mapper.loginInsert(userVO);
            ...
            if(i>0){
                ...
                //将手机号存入到redis中
                redisHelp.setRedis(userVO.getUserCode(), 60, j+"");
            }
    ...
    public @ResponseBody Dto register_1(String user,String code) throws Exception{
        String key=redisHelp.getRedis(user);
        if(key.equals(code)){
            //证明验证码正确，修改数据库的标识
